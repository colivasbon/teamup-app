...
const IconHome = ({ a }) => (
  <svg width="23" height="23" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={a?2.4:1.8} strokeLinecap="round" strokeLinejoin="round">
    <path d="M3 9.5L12 3l9 6.5V20a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V9.5z" fill={a?'currentColor':'none'}/>
    {a && <path d="M9 21V12h6v9" fill="none" stroke="white" strokeWidth="2"/>}
  </svg>
)
const IconSearch = ({ a }) => (
...
const IconMoments = ({ a }) => (
  <svg width="23" height="23" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={a?2.3:1.8} strokeLinecap="round" strokeLinejoin="round">
    <rect x="3" y="3" width="18" height="18" rx="3" fill={a?'currentColor':'none'}/>
    <circle cx="8.5" cy="8.5" r="1.5" fill={a?'white':'currentColor'} stroke="none"/>
    <path d="M21 15 L16 10 L5 21" stroke={a?'white':'currentColor'} strokeWidth={a?2.2:1.8} fill="none"/>
  </svg>
)
const IconCreate = ({ a }) => (
  <div style={{
        ...
        {NAV_ITEMS.map(({ href, label, icon }) => {
          ...
          return (
            <Link key={href} href={href} className={`nav-item${active ? ' active' : ''}`}>
              {/* Wrapper con position:relative siempre, para el badge */}
