-- ============================================================
-- TeamUp — Fix RLS v9.2 + columnas nuevas
-- Ejecutar en: Supabase Dashboard → SQL Editor
-- ============================================================

-- ─── 1. Participantes sin status → 'joined' ──────────────
UPDATE public.event_participants SET status = 'joined' WHERE status IS NULL;
ALTER TABLE public.event_participants ALTER COLUMN status SET DEFAULT 'joined';

-- ─── 2. Nuevas columnas en moments ───────────────────────
ALTER TABLE public.moments ADD COLUMN IF NOT EXISTS province text;
ALTER TABLE public.moments ADD COLUMN IF NOT EXISTS event_id uuid REFERENCES public.events(id) ON DELETE SET NULL;
...
CREATE POLICY "moments_insert_auth" ON public.moments FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "likes_select_all"  ON public.moment_likes FOR SELECT USING (true);
CREATE POLICY "likes_insert_auth" ON public.moment_likes FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "likes_delete_own"  ON public.moment_likes FOR DELETE USING (auth.uid() = user_id);
...
-- ─── 7. Storage bucket moment-images ─────────────────────
INSERT INTO storage.buckets (id, name, public)
  VALUES ('moment-images', 'moment-images', true)
  ON CONFLICT (id) DO NOTHING;

DROP POLICY IF EXISTS "moment-images public read"   ON storage.objects;
DROP POLICY IF EXISTS "moment-images upload auth"   ON storage.objects;

CREATE POLICY "moment-images public read"
  ON storage.objects FOR SELECT
  USING (bucket_id = 'moment-images');

CREATE POLICY "moment-images upload auth"
